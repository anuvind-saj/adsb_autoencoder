...
\begin{center}
  ...
  Code-verified results from the \texttt{adsb\_autoencoder} project.\\[0.25em]
  \url{https://github.com/anuvind-saj/adsb_autoencoder}
\end{center}
\vspace{1em}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item A \textbf{phase-aware 1D U-Net} (\texttt{IQAutoencoder}; 4.19\,M
        parameters) with composite loss over IQ channels, magnitude envelopes,
        and circular phase, designed for OOK carrier structure.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \texttt{compare\_decodings.py} & Compare raw vs processed (two \texttt{.bin} files) \\
    \bottomrule
  \end{tabular}
\end{table}
...
Source code, evaluation scripts, paper figures, and this report are available
at \cite{adsb_repo}:
\url{https://github.com/anuvind-saj/adsb_autoencoder}.

\subsection{Evaluation Command}

\texttt{compare\_decodings.py} requires \textbf{two} RTL-SDR \texttt{.bin} files:
raw (baseline) and processed (from \texttt{live\_bridge.py}).
...
Reports CRC-valid frame count, unique ICAO addresses, preamble candidates,
and delta vs.\ raw.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \texttt{compare\_decodings.py} & Compare raw vs processed decode evaluation \\
...
\texttt{WHITEPAPER\_BLUEPRINT.md} in the repository~\cite{adsb_repo}. That file is a
